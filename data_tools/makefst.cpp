...
int unpack(const char* fst_file, const char* out_path)
{
    ...
    auto ff = std::make_unique<FastFile>();
    ...
    {
        auto hash = fh[j].pfe->hash;
        auto fname = std::string(fh[j].pfe->name);

        auto fHandle = ff->openFast(hash, fname.c_str());
        if (fHandle == -1)
        {
...
int pack(const char* in_path, const char* fst_file, const char* mount, const char* rsp_file, bool b_compress)
{
    ...
    if (!rsp_file)
    {
        ...
        while (!dirs2process.empty())
        {
            ...
            {  // can happen first time when current path is ""
