...
ase2tgl: $(SRC_DATA_ROOT)/tgl/*.ini  $(SRC_DATA_ROOT)/tgl/*.ase
	$(MC2_BIN)/aseconv -prefix-path $(SRC_DATA_ROOT)/tgl/ -dst-path $(SRC_DATA_ROOT)/tgl/
	$(TOUCH) ase2tgl

tgl.rsp: ase2tgl
	$(MC2_BIN)/makersp -rsp tgl.rsp -p $(SRC_DATA_ROOT) -w tgl/\*.tgl -w tgl/\*.agl -w tgl/128/\*.tga -w tgl/\*.ini

tgl.fst: tgl.rsp
	$(MC2_BIN)/makefst -f tgl.fst -p $(SRC_DATA_ROOT) -rsp tgl.rsp -m data
