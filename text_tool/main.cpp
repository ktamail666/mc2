...
bool gos_save_glyphs(const char* glyphFile, const gosGlyphInfo& gi)
{
    ...
    {
        num_structs_written += fwrite(&gi.glyphs_[num_structs_written], sizeof(gosGlyphMetrics), gi.num_glyphs_ - num_structs_written, glyph_info);
    }
...
int main(int argc, char** argv)
{
    ...
    printf("family: %s, style: %s, size:%d\n", TTF_FontFaceFamilyName(font), TTF_FontFaceStyleName(font), fontSize);
    ...
    for (int i = 0; i < NUM_GLYPHS; ++i)
    {
        ...
        TTF_GlyphMetrics(font, glyph_id, &gm[i].minx, &gm[i].maxx, &gm[i].miny, &gm[i].maxy, &gm[i].advance);
    ...
    SDL_Surface* fontTexture = SDL_CreateRGBSurface(0, finalTextureWidth, finalTextureHeight, 32, 0x00FF0000, 0x0000FF00, 0x000000FF, 0xFF000000);
