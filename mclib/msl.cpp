...
//It does malloc and the newShape is a completely independent copy
//That must be destroyed or memory will leak!
TG_MultiShapePtr TG_TypeMultiShape::CreateFrom()
{
    ...
    //Setup Hierarchy again because pointers are not valid!
    for (int i = 0; i < numTG_TypeShapes; i++)
    {
        ...
        newShape->listOfShapes[i].baseRotation.x = newShape->listOfShapes[i].baseRotation.y = newShape->listOfShapes[i].baseRotation.z = 0.0f;
...
long TG_TypeMultiShape::LoadBinaryCopy(const char* fileName)
{
    ...
    if (version == CURRENT_SHAPE_VERSION)
    {
        ...
        for (int i = 0; i < numTG_TypeShapes; i++)
        {
            ...
            if ((S_strnicmp(listOfTypeShapes[i]->getNodeId(), "_PAB", 4) != 0) && (S_strnicmp(listOfTypeShapes[i]->getNodeId(), "LOS_", 4) != 0)
...
//			PAUSE(("Unable to locate shape data for %s.  No ASE or TGL File",fileName));
    ...
    else
    {
        ...
        if (armProvider)
        {
            armAssetPtr = armProvider->OpenAsset(fileName, AssetType_Physical, ProviderType_Primary);
        ...
        if (numTextures)
        {
            ...
            for (int i = (numTextures / 2); i < numTextures; i++)
            {
                ...
                strncpy(
                    listOfTextures[i].textureName, listOfTextures[i - (numTextures / 2)].textureName,
            ...
            if (childChain[j]->calcedThisFrame != turn)
            {
                if (j == curChild)  //This is the ROOT Node.
                {
                    ...
                    if (childChain[j]->currentAnimation)
                    {
                        ...
                        if ((childChain[j]->baseRotation.w == 1.0f) && (childChain[j]->baseRotation.x == 0.0f) && (childChain[j]->baseRotation.y == 0.0f)
