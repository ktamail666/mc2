...
void drawEmptyRect(const GUI_RECT& area, unsigned long leftTopBorderColor, unsigned long rightBottomBorderColor)
{
    gos_VERTEX v[4];
    ...
    if ((clientArea.left < 0 && clientArea.right < 0) || (clientArea.right > Environment.screenWidth && clientArea.left > Environment.screenWidth)
        || (clientArea.top < 0 && clientArea.bottom < 0) || (clientArea.top > Environment.screenHeight && clientArea.bottom > Environment.screenHeight))
        return;
...
void drawShadowText(long colorTop, long colorShadow, HGOSFONT3D font, long left, long top, bool proportional, const char* text, bool bold, float scale)
{
