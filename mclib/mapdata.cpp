...
{
    return file->writePacket(whichPacket, (unsigned char*)blocks, Terrain::realVerticesMapSide * Terrain::realVerticesMapSide * sizeof(PostcompVertex));
}
...
void MapData::highlightAllTransitionsOver2(void)
{
    ...
    for (long y = 0; y < (Terrain::realVerticesMapSide - 1); y++)
    {
        for (long x = 0; x < (Terrain::realVerticesMapSide - 1); x++)
        {
            ...
            //-------------------------------------------------------------------------------
            long totalNotEqual1 =
                abs((pVertex1->terrainType != pVertex2->terrainType) + (pVertex3->terrainType != pVertex4->terrainType)
                    + (pVertex2->terrainType != pVertex4->terrainType));

            long totalNotEqual2 =
                abs((pVertex2->terrainType != pVertex3->terrainType) + (pVertex1->terrainType != pVertex4->terrainType)
...
void MapData::calcTransitions()
{
    ...
    for (long y = 0; y < (Terrain::realVerticesMapSide - 1); y++)
    {
        for (long x = 0; x < (Terrain::realVerticesMapSide - 1); x++)
        {
            ...
            DWORD terrainType = pVertex1->terrainType + (pVertex2->terrainType << 8) + (pVertex3->terrainType << 16) + (pVertex4->terrainType << 24);
    ...
    DWORD terrainType = MC_BLUEWATER_TYPE + (MC_BLUEWATER_TYPE << 8) + (MC_BLUEWATER_TYPE << 16) + (MC_BLUEWATER_TYPE << 24);
...
void MapData::calcLight(void)
{
    ...
    for (long i = 0; i < totalVertices; i++)
    {
        ...
        if (((diskMapIndex - Terrain::realVerticesMapSide - 1) < 0) || ((diskMapIndex - Terrain::realVerticesMapSide) < 0) || ((diskMapIndex - 1) < 0))
        {
            ...
            ((diskMapIndex + 1) >= totalVertices) || ((diskMapIndex + Terrain::realVerticesMapSide) >= totalVertices)
        ...
        else
        {
            ...
            currentVertex->vertexNormal.x =
                normals[0].x + normals[1].x + normals[2].x + normals[3].x + normals[4].x + normals[5].x + normals[6].x + normals[7].x;
            currentVertex->vertexNormal.y =
                normals[0].y + normals[1].y + normals[2].y + normals[3].y + normals[4].y + normals[5].y + normals[6].y + normals[7].y;
            currentVertex->vertexNormal.z =
                normals[0].z + normals[1].z + normals[2].z + normals[3].z + normals[4].z + normals[5].z + normals[6].z + normals[7].z;
...
float MapData::terrainAngle(const Stuff::Vector3D& position, Stuff::Vector3D* normal)
{
    ...
    if (((meshOffset.x + 1) >= Terrain::realVerticesMapSide) || ((meshOffset.y + 1) >= Terrain::realVerticesMapSide) || (meshOffset.x < 0)
        || (meshOffset.y < 0))
        return (0.0f);
...
float MapData::terrainLight(const Stuff::Vector3D& position)
{
    ...
    weightedNormal.x =
        (pVertex1->vertexNormal.x * dist1) + (pVertex2->vertexNormal.x * dist2) + (pVertex3->vertexNormal.x * dist3) + (pVertex4->vertexNormal.x * dist4);

    weightedNormal.y =
        (pVertex1->vertexNormal.y * dist1) + (pVertex2->vertexNormal.y * dist2) + (pVertex3->vertexNormal.y * dist3) + (pVertex4->vertexNormal.y * dist4);

    weightedNormal.z =
        (pVertex1->vertexNormal.z * dist1) + (pVertex2->vertexNormal.z * dist2) + (pVertex3->vertexNormal.z * dist3) + (pVertex4->vertexNormal.z * dist4);
