...
//---------------------------------------------------------------------------
HeapManager::operator MemoryPtr()
{
    return getHeapPtr();
}

//---------------------------------------------------------------------------
MemoryPtr HeapManager::getHeapPtr()
{
    ...
    for (long i = 0; i < MAX_HEAPS; i++)
    {
        ...
        if (currentHeap)
        {
            ...
            if (currentHeap->heapType() == 1)
            {
                ...
                sprintf(
                    msg, "Frag Level: %f       PercentFree: %f", (float(userHeap->coreLeft()) / float(userHeap->totalCoreLeft())),
