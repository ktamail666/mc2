...
const char* ComponentFormString[] = { "Simple",       "Cockpit",         "Sensors",       "Actuator", "Engine",  "HeatSink", "Weapon",
                                      "EnergyWeapon", "BallisticWeapon", "MissileWeapon", "Ammo",     "JumpJet", "Case",     "LifeSupport",
                                      "Gyroscope",    "PowerAmplifier",  "ECM",           "Probe",    "Jammer",  "Bulk",     NULL };

const char* WeaponRangeString[] = { "short", "medium", "long" };
...
long MasterComponent::saveMasterList(const char* fileName, long listSize, float baseSensorRange)
{
    ...
    sprintf(
        dataLine,
        "// Component Table,,abbr,type,crits,?,tons,RP,head,CT,LT,RT,LA,RA,LL,RL,Vehicle?,Fit both?,Side,Fit IS?,art,disable,BR,Damage,Recycle,heat,#miss,miss "
        "type,min,short,med,long,Type,Field,Master ID,Fields,4 = LBX 8 = artillery");
    componentFile.writeLine(dataLine);
