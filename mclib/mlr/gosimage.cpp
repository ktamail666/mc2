...
GOSImage::GOSImage(const char* iName)
    : Plug(DefaultData)
{
    imageName = iName;
...
GOSImage::GOSImage(DWORD iHandle)
    : Plug(DefaultData)
{
    char str[20];
...
GOSImage::GOSImage(const char* name, gos_TextureHints hints)
    : Plug(DefaultData)
{
    imageName = name;
...
unsigned char* GOSImage::GetImagePtr()
{
