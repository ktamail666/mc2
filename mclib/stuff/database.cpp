...
public:
    ...
    size_t m_idOffsets[e_DataBlockSize],  // Offsets to DatabaseRecords ( sorted by index )
...
{
public:
    Record(const RecordHandle* handle, DWORD record_hash, DWORD name_length);
    void Unhook(const RecordHandle* handle);

    __int64 m_lastModified;  // Time record was last modifyed

    size_t m_nextIDRecord,  // offset to chain of records that share the same hash
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
//
Record::Record(const RecordHandle* handle, DWORD record_hash, DWORD name_length)
{
    Check_Pointer(this);
    Check_Object(handle);
...
void RecordHandle::Add()
{
    ...
    //
    Verify(!m_record);
    Record* data = new (new BYTE[sizeof(*m_record) + m_length + name_length]) Record(this, record_hash, name_length);
    Check_Object(data);
...
bool RecordHandle::FindID()
{
    ...
    while (record)
    {
        record = reinterpret_cast<Record*>((size_t)record + m_databaseHandle->m_baseAddress);
        Check_Object(record);
        if (record->m_ID == m_ID)
        {
...
bool RecordHandle::ReadAndNext()
{
    ...
    m_databaseHandle->m_currentPointer = reinterpret_cast<Record*>(m_databaseHandle->m_currentPointer->m_nextIDRecord);
    if (!m_databaseHandle->m_currentPointer)
    {
        while (++m_databaseHandle->m_currentRecord < Database::e_DataBlockSize)
        {
            data = reinterpret_cast<Record*>(db->m_idOffsets[m_databaseHandle->m_currentRecord]);
            if (data)
            {
                m_databaseHandle->m_currentPointer = reinterpret_cast<Record*>((size_t)data + m_databaseHandle->m_baseAddress);
                return true;
            }
        }
    ...
    m_databaseHandle->m_currentPointer = reinterpret_cast<Record*>((size_t)m_databaseHandle->m_currentPointer + m_databaseHandle->m_baseAddress);
    return true;
}
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
//
DatabaseHandle::DatabaseHandle(const char* filename, bool read_only)
{
    //
    //-----------------------------------------------------
    ...
    if (m_readOnly)
    {
        if (gos_DoesFileExist(filename))
        {
            ...
            DWORD size;
            m_handle = gos_OpenMemoryMappedFile(filename, reinterpret_cast<BYTE**>(&m_dataBase), &size);
    ...
    else
    {
        ...
        {
            size_t size;
            gos_GetFile(filename, reinterpret_cast<BYTE**>(&m_dataBase), &size);
            FileStream::IsRedirected = false;
...
void DatabaseHandle::Save()
{
    ...
    {
        DWORD
        m_ID, m_offset, m_nextIDRecord, m_nextNameRecord;
        Record* m_data;
    };
    ...
    for (DWORD i = 0; i < Database::e_DataBlockSize; ++i)
    {
        ...
        if (old_record)
        {
            ...
            while (old_record)
            {
                ...
                offset += sizeof(*old_record) + old_record->m_length + old_record->m_nameLength;
    ...
    for (int i = 0; i < m_dataBase->m_numberOfRecords; ++i, ++new_record)
    {
        ...
        //------------------------------------------
        //
        db_file.WriteBytes(old_record, sizeof(*old_record) + old_record->m_length + old_record->m_nameLength);
...
void DatabaseHandle::First()
{
    ...
    for (DWORD i = 0; i < Database::e_DataBlockSize; i++)
    {
        Record* data = reinterpret_cast<Record*>(m_dataBase->m_idOffsets[i] + m_baseAddress);
        if (data)
        {
            m_currentRecord  = i;
