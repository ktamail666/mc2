...
inline Stuff::IteratorPosition GetHashValue(int value_to_hash)
{
    return value_to_hash;
}
};

// sebi same crosserefence in header problem, need to add declaration here explicitly
namespace GetHashFunctions {
...
private:
    ...
    //--------------------------------------------------------------------
    //
    virtual SortedChain* MakeSortedChain();

    virtual IteratorPosition GetHashIndex(const void* value);

    void BuildHashTable();

    bool CheckForPrimeSize();
};
...
HashOf<T, V>::~HashOf()
{
    ...
    for (int i = 0; i < hashTableSize; i++)
    {
        ...
        if (hashTable[i] != NULL)
        {
            ...
            SortedChainIteratorOf<T, V> iterator((SortedChainOf<T, V>*)hashTable[i]);
...
template <class T, class V>
SortedChain* HashOf<T, V>::MakeSortedChain()
{
    return new SortedChainOf<T, V>(GetReleaseNode(), HasUniqueEntries());
}

template <class T, class V>
IteratorPosition HashOf<T, V>::GetHashIndex(const void* value)
{
    Check_Pointer(value);
...
private:
    ...
    void DeleteSortedChainIterator();
...
template <class T, class V>
class HashIteratorOf : public HashIterator
{
public:
    //
    //--------------------------------------------------------------------
...
template <class T, class V>
Iterator* HashIteratorOf<T, V>::MakeClone()
{
