...
enum
{
    SNAN_NEGATIVE_INT32 = 0xffb1ffb1
};
...
#define DECLARE_TIMER(scope, t) scope __int64 t##FrameTime
#define DEFINE_TIMER(c, t)      __int64 c::t##FrameTime
...
void Flood_Memory_With_NAN(void* where, size_t how_much);
...
protected:
    Signature();
    // because objects inherit from it
    virtual ~Signature();
...
//
#if !defined(Spew)
inline void Spew(const char* group, int value)
{
    SPEW((group, "%d+", value));
}

inline void Spew(const char* group, float value)
{
    SPEW((group, "%f+", value));
}

inline void Spew(const char* group, const char* value)
{
    SPEW((group, "%s+", value));
}
#endif
