...
{
public:
    Directory(char* find_files, bool find_directories = false);
    ~Directory();
...
public:
    ...
    FileStream();
    explicit FileStream(const char* file_name, WriteStatus writable = ReadOnly);
    ~FileStream();

    void Open(const char* file_name = NULL, WriteStatus writable = ReadOnly);
    void Close();
...
public:
    ...
    {
        STOP(("No implementation possible for FileStream::SetPointer(void*)"));
    }
    ...
    MemoryStream& ReadBytes(void* ptr, DWORD number_of_bytes);
    MemoryStream& WriteBytes(const void* ptr, DWORD number_of_bytes);
...
MString* StripExtension(MString* file_name);
MString* IsolateDirectory(MString* file_name);
MString* StripDirectory(MString* file_name);

bool CreateDirectories(const char* directories);
}
