...
template <class V>
class TreeNodeOf : public TreeNode
{
public:
...
template <class V>
MemoryBlock* TreeNodeOf<V>::allocatedMemory = NULL;
template <class V>
CollectionSize TreeNodeOf<V>::allocationCount = 0;
...
template <class V>
void* TreeNodeOf<V>::operator new(size_t)
{
    Verify(allocationCount >= 0);
    if (allocationCount++ == 0)
    {
        allocatedMemory = new MemoryBlock(
            sizeof(TreeNodeOf<V>), TreeNode_MemoryBlock_Allocation, TreeNode_MemoryBlock_Allocation, "Stuff::TreeNodeOf", Stuff::ConnectionEngineHeap);
...
template <class V>
void TreeNodeOf<V>::operator delete(void* where)
{
    Check_Object(allocatedMemory);
...
private:
    ...
    //--------------------------------------------------------------------
    //
    virtual TreeNode* MakeTreeNode(Plug* plug, const void* value);
...
private:
    ...
    //--------------------------------------------------------------------
    //
    TreeNode* MakeTreeNode(Plug* plug, const void* value)
    {
...
template <class T, class V>
int TreeOf<T, V>::CompareTreeNodes(TreeNode* node1, TreeNode* node2)
{
...
template <class T, class V>
int TreeOf<T, V>::CompareValueToTreeNode(const void* value, TreeNode* node)
{
    Check_Pointer(value);
...
template <class T, class V>
class TreeIteratorOf : public TreeIterator
{
public:
    //
    //--------------------------------------------------------------------
...
template <class T, class V>
Iterator* TreeIteratorOf<T, V>::MakeClone()
{
