...
static wchar_t* __cdecl wideRev(wchar_t* string)
{
...
static wchar_t* __cdecl wideStrStr(const wchar_t* wcs1, const wchar_t* wcs2)
{
...
static int __cdecl wideToInt(const wchar_t* nptr)
{
    char astring[20];
...
static long __cdecl atolong(const char* nptr)
{
    int c;      /* current char */
...
static char* __cdecl StrRev(char* string)
{
    char* start = string;
    char* left  = string;
    char ch;
...
inline const EChar* KSInc(const EChar* p)
{
    return p + 1;
}
inline size_t ECharLen(const EChar* p)
{
    return 2;
}
...
__inline char* __cdecl KSInc(const char* _pc)
{
    return (char*)(_pc + 1);
}
inline int ECharLen(const char* x)
{
    return 1;
}
#endif
...
#define A2W(p_String)                \
    (((const char*)p_String == NULL) \
         ? NULL                      \
         : (EString::ToUnicode((LPWSTR)_alloca((lstrlenA(p_String) + 1) * 2), (const unsigned char*)p_String, (lstrlenA(p_String) + 1) * 2)))

// if this doesn't want to link properly, this will have to become
// a macro.
/////////////////////////////////////////////////////////////////
inline wchar_t* EString::ToUnicode(wchar_t* p_Buffer, const unsigned char* p_Str, int Num_Chars)
{
    gosASSERT(p_Buffer);
    gosASSERT(p_Str);
...
void EString::Alloc(int Min_Amount)
{
    ...
    m_pBuffer = (EString::EBuffer*)malloc(sizeof(EBuffer) + (Min_Amount) * sizeof(EChar));
...
void EString::Replace(int Start_Index, const EChar* p_String)
{
    ...
    if (Start_Index <= Cur_Len)
    {
        ...
        else  // need to reallocate here
        {
            ...
            memcpy(m_pBuffer->Data(), p_Tmp->Data(), (Cur_Len + 1) * sizeof(EChar));
...
void EString::Insert(int Start_Index, const EChar* p_String)
{
    if (Start_Index != INVALID_INDEX && Start_Index <= StrSize(Data()))
    {
        ...
        memmove(m_pBuffer->Data() + (Start_Index + Length), p_Tmp->Data() + (Start_Index), (p_Tmp->m_Data_Length - Start_Index) * sizeof(EChar));
...
bool EString::Remove(int Start_Index, int End_Index)
{
    ...
    if (Start_Index <= End_Index && End_Index < m_pBuffer->m_Data_Length)
    {
        ...
        memmove(m_pBuffer->Data() + Start_Index, m_pBuffer->Data() + End_Index + 1, (StrSize(m_pBuffer->Data()) - End_Index - 1) * sizeof(EChar));
...
void EString::Format(const EChar* p_Str, ...)
{
    ...
    for (const EChar* p_Tmp = p_Str; *p_Tmp != '\0'; p_Tmp = KSInc(p_Tmp))
    {
        ...
        for (; *p_Tmp != '\0'; p_Tmp = KSInc(p_Tmp))
        {
            ...
            else if (*p_Tmp == '-' || *p_Tmp == '+' || *p_Tmp == '0' || *p_Tmp == ' ')
                ;
...
/////////////////////////////////////////////////////////////////
EString operator+(const EString& Begin_String, const EString& End_String)
{
    return Begin_String + (const EChar*)(End_String.m_pBuffer->Data());
}

/////////////////////////////////////////////////////////////////
EString operator+(const EChar* p_Begin_String, const EString& End_String)
{
    EString Ret_String;
    ...
    memcpy(Ret_String.m_pBuffer->Data() + Length, End_String.m_pBuffer->Data(), End_String.m_pBuffer->m_Data_Length);
...
EString operator+(const EString& Begin_String, const EChar* p_End_String)
{
    ...
    memcpy(Ret_String.m_pBuffer->Data(), Begin_String.m_pBuffer->Data(), Begin_String.m_pBuffer->m_Data_Length);

    memcpy(Ret_String.m_pBuffer->Data() + Begin_String.m_pBuffer->m_Data_Length, p_End_String, Length);
...
int EString::Find(const EString& Str_To_Find, int Start_Index) const
{
    ...
    EChar* p_Tmp = KStrStr(m_pBuffer->Data() + Start_Index, Str_To_Find.m_pBuffer->Data());
...
int EString::Find(const EChar* p_Str_To_Find, int Start_Index) const
{
    ...
    EChar* p_Tmp = KStrStr(m_pBuffer->Data() + Start_Index, p_Str_To_Find);
...
EString EString::SubString(int Start_Index, int End_Index) const
{
    ...
    // (Start == End)
    gosASSERT(Start_Index <= End_Index && End_Index < m_pBuffer->m_Data_Length);
    ...
    memcpy(p_Dest, p_Src, (End_Index - Start_Index + 1) * sizeof(EChar));
