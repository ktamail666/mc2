...
public:
    ...
    static Event* Make(Stuff::MemoryStream* stream, int gfx_version);

    void Save(Stuff::MemoryStream* stream);

    Stuff::Scalar m_time;
    unsigned m_flags, m_effectID;
    Stuff::LinearMatrix4D m_localToParent;

protected:
    Event(Stuff::MemoryStream* stream, int gfx_version);
};
...
#endif
{
    ...
    //
protected:
    Effect__Specification(Stuff::RegisteredClass::ClassID class_id, Stuff::MemoryStream* stream, int gfx_version);

public:
    Effect__Specification(Stuff::RegisteredClass::ClassID class_id = gosFX::EffectClassID);
    virtual ~Effect__Specification();

    static Effect__Specification* Create(Stuff::MemoryStream* stream, int gfx_version);

    typedef Effect__Specification* (*Factory)(Stuff::MemoryStream* stream, int gfx_version);

    static Effect__Specification* Make(Stuff::MemoryStream* stream, int gfx_version);

    virtual void Save(Stuff::MemoryStream* stream);

    virtual void BuildDefaults();
...
public:
    ...
    {
        Stuff::Time m_time;
        Stuff::Scalar m_seed,  // 0 <= m_seed <= 1
            m_age, m_ageRate;
        const Stuff::LinearMatrix4D* m_parentToWorld;
        Stuff::OBB* m_bounds;

        ExecuteInfo(Stuff::Time time, const Stuff::LinearMatrix4D* parent_to_world, Stuff::OBB* bounds, Stuff::Scalar seed = -1.0f)
        {
            m_time          = time;
            m_seed          = seed;
    ...
    private:
        ExecuteInfo(Stuff::Scalar time, const Stuff::LinearMatrix4D* parent_to_world, Stuff::OBB* bounds, Stuff::Scalar seed = -1.0f);
    };
...
public:
    ...
    //
protected:
    Effect(ClassData* class_data, Specification* spec, unsigned flags);

public:
    ~Effect();

    typedef Effect* (*Factory)(Specification* spec, unsigned flags);

    static Effect* Make(Specification* spec, unsigned flags);
