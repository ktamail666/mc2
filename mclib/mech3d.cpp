...
void Mech3DAppearance::init(AppearanceTypePtr tree, GameObjectPtr obj)
{
    ...
    leftFootPoofDraw[0] = leftFootPoofDraw[1] = leftFootPoofDraw[2] = rightFootPoofDraw[0] = rightFootPoofDraw[1] = rightFootPoofDraw[2] = false;
...
{
    if ((currentGestureId == GestureStand) || (currentGestureId == GestureWalk) || (currentGestureId == GestureReverse) || (currentGestureId == GestureRun)
...
void Mech3DAppearance::resetPaintScheme(DWORD red, DWORD green, DWORD blue)
{
    ...
    if (fileExists(textureName))
    {
        DWORD textureInstanceAlreadyExists =
            mcTextureManager->textureInstanceExists(textureName, gos_Texture_Solid, gosHint_DisableMipmap | gosHint_DontShrink, paintInstance);
...
void Mech3DAppearance::setObjectParameters(const Stuff::Vector3D& pos, float Rot, long sel, long team, long homeRelations)
{
    ...
    if ((currentGestureId == 23) || (currentGestureId == 24) || (currentGestureId == 21) || (currentGestureId == 22) || (currentGestureId == 14)
        || (currentGestureId == 15))
    {
    }
    else
            ...
            for (long i = 0; i < mechShape->GetNumTextures(); i++)
            {
                ...
                if (fileExists(textureName))
                {
                    if (S_strnicmp(txmName, "a_", 2) == 0)
                    {
                        ...
                        {
                            localTextureHandle =
                                mcTextureManager->loadTexture(textureName, gos_Texture_Alpha, gosHint_DisableMipmap | gosHint_DontShrink, true);
                    ...
                    else
                    {
                        ...
                        {
                            localTextureHandle =
                                mcTextureManager->loadTexture(textureName, gos_Texture_Solid, gosHint_DisableMipmap | gosHint_DontShrink, true);
                            mechShape->SetTextureHandle(i, localTextureHandle);
...
void Mech3DAppearance::updateGeometry(void)
{
    ...
    if (visible && (sensorLevel > 4) && !InEditor && useNonWeaponEffects)
    {
        ...
        if (!rightFootDone0 && (currentFrame >= (mechType->gestures[currentGestureId].rightFootDownFrame0 - FOOTPRINT_SLOP))
        ...
        if (!rightFootDone1 && (currentFrame >= (mechType->gestures[currentGestureId].rightFootDownFrame1 - FOOTPRINT_SLOP))
        ...
        if (!leftFootDone0 && (currentFrame >= (mechType->gestures[currentGestureId].leftFootDownFrame0 - FOOTPRINT_SLOP))
        ...
        if (!leftFootDone1 && (currentFrame >= (mechType->gestures[currentGestureId].leftFootDownFrame1 - FOOTPRINT_SLOP))
...
long Mech3DAppearance::update(bool animate)
{
    ...
    if ((currentGestureId == GestureHitFront) || (currentGestureId == GestureHitBack) || (currentGestureId == GestureHitLeft)
        || (currentGestureId == GestureHitRight))
    {
