...
const int constantSizes[] = {
    sizeof(float), sizeof(int),     2 * sizeof(float), 3 * sizeof(float), 4 * sizeof(float), sizeof(int) * 2,   sizeof(int) * 3,    sizeof(int) * 4,
    sizeof(int),   sizeof(int) * 2, sizeof(int) * 3,   sizeof(int) * 4,   4 * sizeof(float), 9 * sizeof(float), 16 * sizeof(float),
};
...
glsl_shader::Shader_t get_shader_type(GLenum type)
{
    switch (type)
    {
        ...
        case GL_TESS_EVALUATION_SHADER:
            return glsl_shader::DOMAIN;
        case GL_GEOMETRY_SHADER:
            return glsl_shader::GEOMETRY;
        default:
...
void parse_uniforms(GLuint pprogram, glsl_program::UniArr_t* puniforms, glsl_program::SamplerArr_t* psamplers)
{
    ...
    for (GLint i = 0; i < num_uni; ++i)
    {
        ...
        if (-1 == i)
            continue;  // gl_ variable or does not correspond to an active uniform variable name in program
        ...
        if (type >= GL_SAMPLER_1D && type <= GL_SAMPLER_2D_SHADOW)
        {
            ...
            static const char* typeNames[] = { "sampler_1d", "sampler_2d", "sampler_3d", "sampler_cube", "sampler_1d_shadow", "sampler_2d_shadow" };
        ...
        static const char* typeNames[] = { "float", "int  ", "vec2 ", "vec3 ", "vec4 ", "ivec2", "ivec3", "ivec4",
                                           "bool ", "bvec2", "bvec3", "bvec4", "mat2 ", "mat3 ", "mat4 " };
        log_debug("name: %s type: %s  num_el: %d", buf, typeNames[puni->type_], puni->num_el_);
...
glsl_program* glsl_program::makeProgram2(
    const char* name,
    const char* vp,
    const char* hp,
    const char* dp,
    ...
    const char* prefix /*=nullptr*/)
{
    ...
    if (dp)
    {
        dsh = glsl_shader::makeShader(glsl_shader::DOMAIN, dp, prefix);
        if (!dsh)
            return 0;
    }
    ...
    if (gp)
    {
        gsh = glsl_shader::makeShader(glsl_shader::GEOMETRY, gp, prefix);
        if (!gsh)
            return 0;
    }
