...
static gosRenderer* g_gos_renderer = nullptr;
...
GLenum getGLVertexAttribType(gosVERTEX_ATTRIB_TYPE type)
{
    ...
    switch (type)
    {
        case gosVERTEX_ATTRIB_TYPE::BYTE:
            return GL_BYTE;
        case gosVERTEX_ATTRIB_TYPE::UNSIGNED_BYTE:
            return GL_UNSIGNED_BYTE;
        case gosVERTEX_ATTRIB_TYPE::SHORT:
            return GL_SHORT;
        case gosVERTEX_ATTRIB_TYPE::UNSIGNED_SHORT:
            return GL_UNSIGNED_SHORT;
        case gosVERTEX_ATTRIB_TYPE::INT:
            return GL_INT;
        case gosVERTEX_ATTRIB_TYPE::UNSIGNED_INT:
            return GL_UNSIGNED_INT;
        case gosVERTEX_ATTRIB_TYPE::FLOAT:
            return GL_FLOAT;
        default:
...
public:
    ...
    {
        auto* vdecl = new gosVertexDeclaration();
        if (!vdecl)
        {
            return nullptr;
        }
...
public:
    gosMaterialVariation()
        : defines_(nullptr)
        , unique_name_suffix_(nullptr)
    {
    }
    const char* getDefinesString() const
    {
        return defines_;
    }
    const char* getUniqueSuffix() const
    {
        return unique_name_suffix_;
    }
...
static const char* const g_shader_flags[] = { "ALPHA_TEST" };
...
public:
    ...
    {
        gosASSERT(shader);
        auto* pmat = new gosRenderMaterial();
        char vs[256];
        char ps[256];
    ...
    void applyVertexDeclaration()
    {
        ...
        glVertexAttribPointer(pos_loc, 4, GL_FLOAT, GL_FALSE, stride, (void*)nullptr);

        if (color_loc != -1)
        {
            glEnableVertexAttribArray(color_loc);
            glVertexAttribPointer(color_loc, 4, GL_UNSIGNED_BYTE, GL_TRUE, stride, BUFFER_OFFSET(4 * sizeof(float)));
        }

        if (spec_color_and_fog_loc != -1)
        {
            glEnableVertexAttribArray(spec_color_and_fog_loc);
            glVertexAttribPointer(spec_color_and_fog_loc, 4, GL_UNSIGNED_BYTE, GL_TRUE, stride, BUFFER_OFFSET(4 * sizeof(float) + sizeof(uint32_t)));
        }

        if (texcoord_loc != -1)
        {
            glEnableVertexAttribArray(texcoord_loc);
            glVertexAttribPointer(texcoord_loc, 2, GL_FLOAT, GL_FALSE, stride, BUFFER_OFFSET(4 * sizeof(float) + 2 * sizeof(uint32_t)));
        }
    }
...
public:
    ...
    {
        GLuint vb = makeBuffer(GL_ARRAY_BUFFER, nullptr, sizeof(gos_VERTEX) * vertex_capacity, GL_DYNAMIC_DRAW);
        if (!vb)
        {
            return nullptr;
        }
        ...
        if (index_capacity > 0)
        {
            ib = makeBuffer(GL_ELEMENT_ARRAY_BUFFER, nullptr, sizeof(INDEX_TYPE) * index_capacity, GL_DYNAMIC_DRAW);
            if (!ib)
            {
                return nullptr;
            }
        ...
        auto* mesh          = new gosMesh(prim_type, vertex_capacity, index_capacity);
        mesh->vb_           = vb;
        mesh->ib_           = ib;
    ...
    const gos_VERTEX* getVertices() const
    {
        return pvertex_data_;
    }
...
private:
    gosMesh(gosPRIMITIVETYPE prim_type, int vertex_capacity, int index_capacity)
        ...
        , num_vertices_(0)
        , num_indices_(0)
        , pvertex_data_(nullptr)
        , pindex_data_(nullptr)
...
void gosMesh::drawIndexed(gosRenderMaterial* material) const
{
    ...
    glDrawElements(pt, num_indices_, getIndexSizeBytes() == 2 ? GL_UNSIGNED_SHORT : GL_UNSIGNED_INT, nullptr);
...
void gosMesh::drawIndexed(HGOSBUFFER ib, HGOSBUFFER vb, HGOSVERTEXDECLARATION vdecl, gosRenderMaterial* material)
{
    ...
    GLenum pt = GL_TRIANGLES;
    glDrawElements(pt, ib->count_, index_size == 2 ? GL_UNSIGNED_SHORT : GL_UNSIGNED_INT, nullptr);
...
static void makeKindaSolid(Image& img)
{
    ...
    auto* pixels = (DWORD*)img.getPixels();
...
bool gosTexture::createHardwareTexture()
{
    ...
    else if (pcompdata_ && size_ > 0)
    {
        ...
        {
            SPEW(
                ("DBG", "failed to load texture from data, filename: %s, texname: %s\n", filename_ ? filename_ : "NO FILENAME",
                 texname_ ? texname_ : "NO TEXNAME"));
            return false;
        }
    ...
    else
    {
        ...
        auto* pdata  = new DWORD[tex_.w * tex_.h];
...
public:
    ...
    {
        return gi_.font_line_skip_;
    }
    int getFontAscent() const
    {
        return gi_.font_ascent_;
    }
...
public:
    ...
    void deleteFont(gosFont* font)
    {
        ...
        auto it = std::find_if(fontList_.begin(), fontList_.end(), eq);
        if (it != fontList_.end())
        {
            gosFont* font = *it;
    ...
    {
        auto it = std::find(vertexDeclarationList_.begin(), vertexDeclarationList_.end(), vdecl);
    ...
    gosTexture* getTexture(DWORD texture_id)
    {
        ...
        gosASSERT(textureList_[texture_id] != nullptr);
    ...
    void deleteTexture(DWORD texture_id)
    {
        ...
        textureList_[texture_id] = nullptr;
    }
    ...
    gosTextAttribs& getTextAttributes()
    {
        return curTextAttribs_;
    }
    ...
    int getTextRegionWidth()
    {
        return curTextRight_ - curTextLeft_;
    }
    int getTextRegionHeight()
    {
        return curTextBottom_ - curTextTop_;
    }
    ...
    void setRenderViewport(const vec4& vp)
    {
        render_viewport_ = vp;
    }
    vec4 getRenderViewport()
    {
        return render_viewport_;
    }

    const mat4& getProj2Screen()
    {
        return projection_;
    }
    ...
    // debug interface
    void setNumDrawCallsToDraw(uint32_t num)
    {
        num_draw_calls_to_draw_ = num;
    }
    uint32_t getNumDrawCallsToDraw()
    {
        return num_draw_calls_to_draw_;
    }
    void setBreakOnDrawCall(bool b_break)
    {
        break_on_draw_call_ = b_break;
    }
    bool getBreakOnDrawCall()
    {
        return break_on_draw_call_;
    }
    void setBreakDrawCall(uint32_t num)
    {
        break_draw_call_num_ = num;
    }

    graphics::RenderContextHandle getRenderContextHandle()
    {
        return ctx_h_;
    }

    uint32_t getRenderState(gos_RenderState render_state)
    {
        return curStates_[render_state];
    }
...
void gosRenderer::init()
{
    ...
    // y = 1/h; y= 1- y; y =2*y - 1;
    // z = z;
    projection_ = mat4(2.0f / (float)width_, 0, 0.0f, -1.0f, 0, -2.0f / (float)height_, 0.0f, 1.0f, 0, 0, 1.0f, 0.0f, 0, 0, 0.0f, 1.0f);
...
void gosRenderer::applyRenderStates()
{
    switch (renderStates_[gos_State_Culling])
    {
        ...
        case gos_Cull_CCW:
            ...
            break;
        default:
            gosASSERT(0 && "Wrong cull face value");
    }
    ...
    switch (renderStates_[gos_State_ZWrite])
    {
        ...
        case 1:
            ...
            break;
        default:
            gosASSERT(0 && "Wrong depth write value");
    }
    ...
    switch (renderStates_[gos_State_ZCompare])
    {
        ...
        case 2:
            ...
            break;
        default:
            gosASSERT(0 && "Wrong depth test value");
    }
    ...
    switch (renderStates_[gos_State_AlphaMode])
    {
        ...
        case gos_Alpha_AlphaOne:
            ...
            break;
        default:
            gosASSERT(0 && "Wrong alpha mode value");
    }
   ...
   if(enable_alpha_test) {
    ...
    switch (renderStates_[gos_State_Filter])
    {
        ...
        case gos_FilterBiLinear:
            filter = TFM_LINEAR;
            break;
        case gos_FilterTriLinear:
            filter = TFM_LNEAR_MIPMAP_LINEAR;
            break;
    }
    ...
    TexAddressMode address_mode = renderStates_[gos_State_TextureAddress] == gos_TextureWrap ? TAM_REPEAT : TAM_CLAMP_TO_EDGE;
    ...
    for (int i = 0; i < sizeof(tex_states) / sizeof(tex_states[0]); ++i)
    {
        ...
        gosTexture* tex = gosTextureHandle == INVALID_TEXTURE_ID ? nullptr : this->getTexture(gosTextureHandle);
        if (tex)
        {
...
{
    const auto& sh_var = rs[gos_State_Texture] != 0 ? materialDB_.find("gos_tex_vertex")->second : materialDB_.find("gos_vertex")->second;
...
{
    const auto& sh_var = rs[gos_State_Texture] != 0 ? materialDB_.find("gos_tex_vertex_lighted")->second : materialDB_.find("gos_vertex_lighted")->second;
    uint32_t flags     = rs[gos_State_AlphaTest] ? SHADER_FLAG_INDEX_TO_MASK(gosGLOBAL_SHADER_FLAGS::ALPHA_TEST) : 0;
...
void gosRenderer::drawIndexedTris(HGOSBUFFER ib, HGOSBUFFER vb, HGOSVERTEXDECLARATION vdecl, const float* mvp)
{
    ...
    mat4 transform(mvp[0], mvp[1], mvp[2], mvp[3], mvp[4], mvp[5], mvp[6], mvp[7], mvp[8], mvp[9], mvp[10], mvp[11], mvp[12], mvp[13], mvp[14], mvp[15]);
...
int findTextBreak(const char* text, const int count, const gosFont* font, const int region_width, int* out_str_width)
{
    ...
    while (text[pos])
    {
        ...
        {
            if (pos == 0)
            {  // handle case when only one word in line and it does not fit it, just return whole line
                width = cur_width;
                pos   = break_pos;
            }
            break;
...
int calcTextHeight(const char* text, const int count, const gosFont* font, int region_width)
{
    ...
    while (pos < count)
    {
        int num_chars = findTextBreak(text + pos, count - pos, font, region_width, nullptr);
        pos += num_chars;
        num_lines++;
    }
    return num_lines;
...
*/
    ...
    while (pos < count)
    {
        ...
        for (int i = 0; i < num_chars; ++i)
        {
            ...
            addCharacter(
                text_, u0, v0, u1, v1, (float)(x + char_off_x), (float)(y + char_off_y), (float)(x + char_off_x + char_w), (float)(y + char_off_y + char_h));
...
gosFont* gosFont::load(const char* fontFile)
{
    char fname[256];
    char dir[256];
    _splitpath(fontFile, nullptr, dir, fname, nullptr);
    ...
    auto* ptex = new gosTexture(gos_Texture_Alpha, textureName, 0, nullptr, 0, false);
    ...
    void* pInstance /*=0*/)
{
    ...
    auto* ptex = new gosTexture(Format, Hints, w, h, Name);
...
DWORD __stdcall gos_NewTextureFromMemory(
    gos_TextureFormat Format,
    const char* FileName,
    BYTE* pBitmap,
    DWORD Size,
    ...
    void* pInstance /*=0*/)
{
    gosASSERT(pFunc == nullptr);

    auto* ptex = new gosTexture(Format, FileName, Hints, pBitmap, Size, true);
    if (!ptex->createHardwareTexture())
    {
        STOP(("Failed to create texture\n"));
        return INVALID_TEXTURE_ID;
...
DWORD __stdcall gos_NewTextureFromFile(
    gos_TextureFormat Format,
    const char* FileName,
    DWORD Hints /*=0*/,
    gos_RebuildFunction pFunc /*=0*/,
    void* pInstance /*=0*/)
{
    auto* ptex = new gosTexture(Format, FileName, Hints, nullptr, 0, false);
    if (!ptex->createHardwareTexture())
    {
        STOP(("Failed to create texture\n"));
        return INVALID_TEXTURE_ID;
...
void __stdcall gos_SetScreenMode(
    ...
    DWORD bitDepth /*=16*/,
    DWORD Device /*=0*/,
    bool disableZBuffer /*=0*/,
    bool AntiAlias /*=0*/,
    bool RenderToVram /*=0*/,
    bool GotoFullScreen /*=0*/,
    int DirtyRectangle /*=0*/,
    bool GotoWindowMode /*=0*/,
...
void __stdcall gos_SetupViewport(
    bool FillZ,
    float ZBuffer,
    bool FillBG,
    DWORD BGColor,
...
void __stdcall gos_TextSetAttributes(
    HGOSFONT3D FontHandle,
    DWORD Foreground,
    float Size,
    bool WordWrap,
...
GLenum getGLBufferType(gosBUFFER_TYPE type)
{
    ...
    switch (type)
    {
        case gosBUFFER_TYPE::VERTEX:
            t = GL_ARRAY_BUFFER;
            break;
        case gosBUFFER_TYPE::INDEX:
            t = GL_ELEMENT_ARRAY_BUFFER;
            break;
        case gosBUFFER_TYPE::UNIFORM:
            t = GL_UNIFORM_BUFFER;
            break;

        default:
            gosASSERT(0 && "unknowns buffer type");
    }
    return t;
}
...
GLenum getGLBufferUsage(gosBUFFER_USAGE usage)
{
    ...
    switch (usage)
    {
        case gosBUFFER_USAGE::STREAM_DRAW:
            u = GL_STREAM_DRAW;
            break;
        case gosBUFFER_USAGE::STATIC_DRAW:
            u = GL_STATIC_DRAW;
            break;
        case gosBUFFER_USAGE::DYNAMIC_DRAW:
            u = GL_DYNAMIC_DRAW;
            break;
