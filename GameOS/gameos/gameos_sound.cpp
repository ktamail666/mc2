...
public:
    ...
    gosAudio_ChannelInfo* getChannelsInfo()
    {
        return channel_info_;
    }
    const gosAudio_ChannelInfo* getChannelsInfo() const
    {
        return channel_info_;
    }
    ...
    {
        gosASSERT(i < NUM_CHANNELS);
        return &channel_info_[i];
    }
...

    {
        ...
        printf(
            ...
            : (audio_channels > 1) ? "stereo"
                                   : "mono",
            (audio_format & 0x1000) ? "Big-Endian" : "Little-Endian", audio_buffers);
    }
...
void __stdcall gosAudio_CreateResource(
    HGOSAUDIO* hgosaudio,
    enum gosAudio_ResourceType res_type,
    const char* file_name,
    gosAudio_Format* ga_wf,
    void* data,
    int size,
    bool only2D)
{
    ...
    if (needs_conversion)
    {
        ...
        if (0 > SDL_BuildAudioCVT(&cvt, src_fmt, src_channels, src_freq, dst_fmt, dst_channels, dst_freq))
        {
