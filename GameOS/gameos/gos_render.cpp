...
RenderWindow* create_window(const char* pwinname, int width, int height)
{
    ...
    // initialize using 0 video driver
        ...
        for (i = 0; i < n; ++i)
        {
            ...
            else
            {
                ...
                for (j = 0; j < m; ++j)
                {
                    ...
                    SDL_PixelFormatEnumToMasks(mode.format, &bpp, &Rmask, &Gmask, &Bmask, &Amask);
                    fprintf(
                        stderr, "    Mode %d: %dx%d@%dHz, %d bits-per-pixel (%s)\n", j, mode.w, mode.h, mode.refresh_rate, bpp,
...

    {
        window = SDL_CreateWindow(
            pwinname ? pwinname : "--", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, width, height, SDL_WINDOW_OPENGL | SDL_WINDOW_ALLOW_HIGHDPI);
