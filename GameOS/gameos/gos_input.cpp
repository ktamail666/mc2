...
void updateMouseState()
{
    ...
    for (unsigned int b = 0; b < sizeof(buttons) / sizeof(buttons[0]); ++b)
    {
        ...
        {
            mi->button_state_[idx] = (prev_ks == KS_FREE || prev_ks == KS_RELEASED) ? KS_PRESSED : KS_HELD;
        }
        else
        {
            mi->button_state_[idx] = (prev_ks == KS_HELD || prev_ks == KS_PRESSED) ? KS_RELEASED : KS_FREE;
        }
#if 0
