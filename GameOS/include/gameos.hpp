...
//
//
#pragma pack(push, 16)  // TODO I replaced to 16 from 4 because this is default on x64
...
{
    unsigned long x;
    _asm rdtsc _asm mov x, eax return x;
}
#endif  //!_WIN64
...
#define ENTER_DEBUGGER \
    raise(SIGTRAP);  // __builtin_trap(); // GCC \
                     //asm volatile ("int 3;");
#else
#ifdef _WIN64
...
typedef struct
{
    ...
    const char*
        directoryPath;  // If program path end in this directory, move up levels (ie: "\\Code\\AssassinEditor" or "\\Tools\\PixelWhIP"). "!" will force GameOS NOT to change directory at all (Including DEBUG/RELEASE/PROFILE....)
    ...
    bool allowDoubleClicks;   // make double-click messages available to the application (default is false)
                              ...
                              //
    const char*
        RaidFilePath;  //	The path where the exception files for your project should be stored. They will be referred to by hyperlink in the bug description.
    const char*
        RaidCustomFields;  //  A string containing key-value pairs of Raid Database column name and the value for that record. the column must be of type var-char with a width of 32 (standard raid list)
    const char*
        RaidDataSource;  //  The raid data source name (can be retrieved from your odbc control panel or the registry once an rdq has been opened on your machine.
...
typedef struct _gosAudio_Format
{
    ...
    WORD
        cbSize;  // Size, in bytes, of extra format information appended to the end of the WAVEFORMATEX structure. For PCM's, this should be set to 0. For ADPCM, this should be set to 32.
} gosAudio_Format;
...
//
void* __stdcall gos_Malloc(size_t bytes, HGOSHEAP Heap = nullptr);
...
//
const BYTE gosNet_PlayerAdded =
    240;  // FromID will contain the new PlayerID of the player. gos_NetInformation(gos_NumberOfPlayers) will be greater than last game logic.
const BYTE gosNet_PlayerDeleted =
    241;  // FromID will contain the old PlayerID of the player. gos_NetInformation will work with this ID only for the remainder of the current GameLogic. (Packets sent to the player will be ignored)
const BYTE gosNet_GameEnded = 242;  // The server has ended the network game. This is the last packet the game will see.
const BYTE gosNet_Ping =
    243;  // Used to send a ping packet. You will eventually receive back a ping packet containing four DWORD containing the milliseconds time at each end of a 'double ping'.
...
// NOTE: When PrimitiveType is a POINTLIST the lpwIndices and NumberIndices are ignored, it is treated as a list of points
//
void __stdcall gos_RenderVertexBuffer(
    DWORD VertexBufferHandle,
    DWORD StartVertex,
    DWORD NumberVertices,
    WORD* lpwIndices,
    DWORD NumberIndices,
...
enum gos_TextureHints
{
    ...
    gosHint_VideoMemory =
        4,  // Store in "Local" memory on the video card, this is the fastest for the card to texture, but very slow for the processor to page or animate
    gosHint_ReloadFromDisk =
        8,  // GameOS will not store a DWORD copy of the texture, it will reload it from disk on mode changes. (You cannot lock these textures)
    gosHint_PageLast =
        16,  // Frequently used textures should have this set. These textures will be paged ONLY if there are no other HighPriority texture that could be paged
    ...
    gosHint_ForceAlpha =
        8192,  // When gos_Texture_Detect is specified as a format, an alpha texture (KEYED or ALPHA) will be returned. If a solid texture is loaded, the blue channel will be used as the alpha
...
// Type      is how the variables is stored
...
void __stdcall AddDebuggerMenuItem(
    const char* Name,
    bool(__stdcall* CheckMark)(),
    void(__stdcall* Activated)(),
    bool(__stdcall* Greyed)()                                        = 0,
    DWORD(__stdcall* Callback)(const char* Name, DWORD MenuFunction) = 0);
...
#define LOG_START_CHECK GosEventLog::LogStartPoint();
...
public:
    ...
    static void PopStop();
    static char* LogAddr(int part)
    {
        return part ? m_pLogStart2 : m_pLogStart1;
    }
    static DWORD LogBytes(int part)
    {
