...
void aListBox::update()
{
    ...
    { /* if the listbox is invisible, then we probably don't want to update it either */
        ...
        for (int i = 0; i < itemCount; i++)
        {
            ...
            {
                items[i]->showGUIWindow(true);
            }
            else
            {
                items[i]->showGUIWindow(false);
            }
        }
        ...
        {
            items[prevCheck]->setCheck(false);
        }
    }
}
...
void aListBox::scroll(int amount)
{
    ...
    if (amount < 0 && items[0]->globalY() > globalY())
    {  // no need to scroll
        return;
    }
...
{
    auto* pItem = new aAnimTextListItem(27333);
    *pItem      = templateItem;
    pItem->setText(text);
...
void aComboBox::update()
{
    if (showWindow)
    {
        ...
        if (0 <= ListBox().GetSelectedItem())
        {
            ...
            aListItem* pListItem = ListBox().GetItem(selectionIndex);
            auto* pTextListItem  = dynamic_cast<aTextListItem*>(pListItem);
            if (nullptr != pTextListItem)
            {
...
void aTextListItem::render()
{
    ...
    font.render(text, location[0].x, y, location[2].x - location[0].x, location[2].y - location[0].y, location[0].argb, false, alignment);
}
