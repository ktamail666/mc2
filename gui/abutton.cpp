...
void aButton::makeUVs(gos_VERTEX* vertices, int State, aButton::aButtonData& data)
{
    ...
    if (left == -1 || top == -1)
    {
        SPEW((nullptr, "makeUVs given an Invalid state\n"));
    }
...
void aButton::init(FitIniFile& buttonFile, const char* str, HGOSFONT3D font)
{
    ...
    if (0 == textureHandle && strlen(data.fileName))
    {
        ...
        gos_LockTexture(gosID, 0, false, &textureData);
...
void aAnimButton::update()
{
    ...
    if (bInside && state != DISABLED && state != HIDDEN)
    {
        ...
        if (userInput->isLeftClick())
        {
            ...
            userInput->getMouseLeftHeld() > holdTime && getParent() && !messageOnRelease && pointInside(userInput->getMouseDragX(), userInput->getMouseDragY()))
        {
...
void aAnimButton::setAnimationInfo(aAnimation* normal, aAnimation* highlight, aAnimation* pressed, aAnimation* disabled)
{
    if (normal)
    {
        normalData.destroy();
