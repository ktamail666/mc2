...
aEdit::aEdit()
{
    if (acp == 0)
    {
        ...
        if (!g_bUseLangDll)
        {
            ...
            if (acp != 949)
            {  // other than Korean version, reduce color value by half so that composition string can be seen
...
void aEdit::handleKeyboard()
{
    ...
    //while( true ) // keep getting keys until buffer is empty
    {
        ...
        if (!handleFormattingKeys(key))
        {
            ...
            if (key == '%' || key == '\\' || key == '/' || key == '\"')
            {  // eat these so andy doesn't try and sprintf on us
                return;
            }
            if (!bWierdChars)
            {
...
void aEdit::render()
{
    ...
    if (nInsertion1 != nInsertion2)
    {
        ...
        GUI_RECT rect = { (int)nXSelStart, (int)globalY(), (int)nXSelEnd, (int)(globalY() + height()) };
        ...
        if (text.Length())
        {
            font.render(&textToDraw[startChar], (int)(globalX() + ENTRY_MARGIN), (int)globalY(), (int)width(), (int)height(), textColor, false, 0);
        }
        ...
        if (text.Length())
        {
            ...
            textToDraw[nMax] = '\0';
            font.render(&textToDraw[nMin], (int)nXSelStart, (int)globalY(), nXSelEnd, (int)height(), selectedColor, false, 0);
            textToDraw[nMax] = tmp;
        }
    }
    else
    {
        ...
        {
            font.render("", (int)(globalX() + ENTRY_MARGIN), (int)globalY(), (int)width(), (int)height(), textColor, false, 0);
        }
    }
...
void aEdit::setFocus(bool bHasFocus)
{
    ...
    if (bAllowIME && bHasFocus)
    {
        ...
        if (!g_bUseLangDll)
        {
            ...
            if (acp != 949)
            {  // other than Korean version, reduce color value by half
...
bool aEdit::handleFormattingKeys(int keycode)
{
    ...
    switch (key)
    {
        ...
        case KEY_RETURN:
        {
            if (gos_GetKeyStatus(KEY_RETURN) == KEY_PRESSED)
            {  // don't want this called more than 1 time if held
                setFocus(false);
            }
            return true;
        }
...
int aEdit::findChar(int nXPos)
{
    ...
    auto* pBegin         = (unsigned char*)&text[0];
