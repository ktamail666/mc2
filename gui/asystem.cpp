...
{
    if (!c)
    {  //If this is NULL, shouldn't we still remove it from the list?
        return;
    }
...
void aObject::setColor(uint32_t newColor, bool bRecurse)
{
    ...
    if (bRecurse)
    {
        ...
        {
            pChildren[i]->setColor(newColor, true);
        }
    }
}
...
void aText::render()
{
    ...
    {
        font.render(text, location[0].x, location[0].y, location[2].x - location[0].x, location[2].y - location[0].y, location[0].argb, false, alignment);
    }
}
