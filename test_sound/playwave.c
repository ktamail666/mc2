...
static int still_playing(void)
{
    ...
    //#else
    return (Mix_Playing(PLAY_CHANNEL));
    //#endif
}
...
static void do_panning_update(void)
{
    ...
    {
        panningok = Mix_SetPanning(PLAY_CHANNEL, leftvol, rightvol);
        if (!panningok)
        {
...
static void do_distance_update(void)
{
    ...
    {
        distanceok = Mix_SetDistance(PLAY_CHANNEL, distance);
        if (!distanceok)
        {
...
static void do_position_update(void)
{
    ...
    {
        positionok = Mix_SetPosition(PLAY_CHANNEL, angle, distance);
        if (!positionok)
        {
...
int main(int argc, char* argv[])
{
    ...
    else
    {
        ...
        printf(
            "Opened audio at %d Hz %d bit %s", audio_rate, (audio_format & 0xFF),
...
//    Mix_ChannelFinished(channel_complete_callback);
    ...
    /* Play and then exit */
    Mix_PlayChannel(PLAY_CHANNEL, wave, loops);
