...
{
    if (pNewMover
        && (pNewMover->getTeamId() != Team::home->getId() && (CONTACT_VISUAL != pNewMover->getContactStatus(Team::home->getId(), true))
...
{
    if (pUnit
        && (pUnit->getTeamId() != Team::home->getId() && (CONTACT_VISUAL != (pUnit->getContactStatus(Team::home->getId(), true)) && !pUnit->isDisabled())))
    {
        setUnit(0);
        return;
    }
    ...
    for (int i = 0; i < 2; i++)
    {
        if (buttons[i].location[0].x <= mouseX && mouseX <= buttons[i].location[2].x && mouseY >= buttons[i].location[0].y
    ...
    // see if its in the scroll bar area
        ...
        mouseX > SCROLLLEFT && mouseX < SCROLLRIGHT && mouseY > SCROLLTOP && mouseY < SCROLLBOTTOM && buttons[0].isEnabled()
        && userInput->getMouseDragX() > SCROLLLEFT && userInput->getMouseDragX() < SCROLLRIGHT && userInput->getMouseDragY() > SCROLLTOP
...
void InfoWindow::drawSkillBar(int skill, float yVal, float height)
{
    ...
    GUI_RECT rect = { (long)(left + InfoWindow::SKILLSKIP), (long)(yVal + InfoWindow::SKILLSKIP + .5f), (long)(left + InfoWindow::SKILLSKIP + SKILLUNITWIDTH),
                      (long)(yVal + height - InfoWindow::SKILLSKIP + .5f) };
