...
void OptionsXScreen::init(FitIniFile* file)
{
    ...
    const char* fileNames[4] = { "mcl_optionsgraphics", "mcl_optionsaudio", "mcl_optionsgameplay", "mcl_optionshotkeys" };
...
void OptionsGraphics::reset(const CPrefs& newPrefs)
{
    ...
    for (int i = 0; i < numResolutionModes; ++i)
    {
        if (resolutionModes[i].xRes == newPrefs.resolutionX && resolutionModes[i].yRes == newPrefs.resolutionY
...
{
    camera.init(rects[1].globalX() + xOffset, rects[1].globalY() + yOffset, rects[1].globalRight() + xOffset, rects[1].globalBottom() + yOffset);
...
void OptionsGamePlay::render()
{
    ...
    long colorToMatch = getButton(MSG_BASE)->isPressed() ? rects[36].getColor() : rects[37].getColor();
    for (int i = 4; i < 36; i++)
    {
        if (rects[i].getColor() == colorToMatch)
        {
            GUI_RECT tmp = { rects[i].globalX() - 2, rects[i].globalY() - 2, rects[i].globalRight() + 1, rects[i].globalBottom() + 1 };
...
void OptionsHotKeys::end()
{
    for (int i = 0; i < hotKeyList.GetItemCount(); i++)
    {
        ...
        if (pItem)
        {
            ...
            MissionInterfaceManager::setHotKey(Command, (gosEnum_KeyIndex)(Key & 0x000ffff), Key & SHIFT, Key & CTRL, Key & ALT);
        }
    }
...
{
    aObject::init(pLeft->globalX(), pLeft->top(), pRight->globalRight() - pLeft->globalX(), pRight->bottom() - pRight->top());
...
void ScrollX::update()
{
    ...
    else if (pointInside(mouseX, mouseY))
    {
        if (userInput->isLeftClick() || gos_GetKeyStatus(KEY_LMOUSE) == KEY_HELD)
        {
            ...
            {
                buttons[2]->press(0);
                if (!buttons[0]->pointInside(mouseX, mouseY) && !buttons[1]->pointInside(mouseX, mouseY)
...
void ScrollX::ResizeAreas(void)
{
    ...
    range = width() - buttons[0]->width() - buttons[1]->width() - buttons[2]->width()
            - 4.f;                             // one scrollwidth for buttons, one for tab. 2 for lines at either end.
