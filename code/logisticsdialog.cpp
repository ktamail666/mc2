...
void LogisticsDialog::update()
{
    ...
    if (userInput->leftMouseReleased() && enterAnim.isDone() && !inside(userInput->getMouseX(), userInput->getMouseY())
...
int LogisticsSaveDialog::init(FitIniFile& file)
{
    if (!s_instance)
    {
        ...
        s_instance->gameListBox.init(
            s_instance->rects[1].globalX(), s_instance->rects[1].globalY(), s_instance->rects[1].width(), s_instance->rects[1].height());
