...
void IfaceCallStrike(long strikeID, Stuff::Vector3D* strikeLoc, GameObjectPtr strikeTarget, bool playerStrike, bool clanStrike, float timeToImpact)
{
...
void ArtilleryChunk::build(long _commanderId, long _strikeType, Stuff::Vector3D location, long _seconds)
{
...
bool ArtilleryType::handleCollision(GameObjectPtr collidee, GameObjectPtr collider)
{
    ...
    if (artillery->getFlag(OBJECT_FLAG_BOOM))
    {
        ...
        if ((collider->getObjectClass() == GATE) || (collider->getObjectClass() == TURRET))
        {
            ...
            (collider->getObjectClass() == BATTLEMECH) && (((MoverPtr)collider)->getMoveType() == MOVETYPE_AIR)
...
long Artillery::update(void)
{
    ...
    if ((info.strike.timeToImpact > 0.0f) || (type->nominalDamage == 0.0))
    {
        ...
        appearance->setObjectParameters(
            iFacePosition, ((ObjectAppearance*)appearance)->rotation, false, getTeamId(), Team::getRelation(getTeamId(), Team::home->getId()));
...
bool Artillery::recalcBounds(CameraPtr myEye)
{
    if (myEye)
    {
        ...
        if (inView)
        {
            ...
            if (((screenPos.x >= 0) && (screenPos.y >= 0) && (screenPos.x <= eye->getScreenResX()) && (screenPos.y <= eye->getScreenResY()))
                || (((iFaceScreen.x >= 0) && (iFaceScreen.y >= 0) && (iFaceScreen.x <= eye->getScreenResX()) && (iFaceScreen.y <= eye->getScreenResY()))))
            {
                inView = true;
            }
            else
...
void Artillery::render(void)
{
    if (inView)
    {
        ...
        if ((!MPlayer && (info.strike.timeToImpact > 0.f))
            || (MPlayer && info.strike.timeToImpact > 0.f && ((teamId == Team::home->getId()) || info.strike.timeToImpact < 3.0f)))
        {
