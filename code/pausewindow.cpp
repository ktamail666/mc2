...
MoveInfo PauseWindow::moveInfo[8] = { 0.0f, 820.f, 0.04f, 820.f, 0.28f, 661.f, .36f, 678.f, .44f, 661.f, .52f, 666.f, .60f, 661.f, 3.00f, 661.f };
...
void PauseWindow::update()
{
    ...
    for (int i = 0; i < buttonCount; i++)
    {
            ...
            && mouseY <= buttons[i].location[1].y)
        {
            if (buttons[i].isEnabled())
            {
                ...
                if (buttons[i].location[0].x >= lastX || lastX >= buttons[i].location[2].x || lastY <= buttons[i].location[0].y
    ...
    if (MPlayer || LogisticsData::instance->isSingleMission() || (S_stricmp("tutorial", campName) == 0))
    {
...
void PauseWindow::render()
{
    ...
    headerFont.render(
        buffer, backgrounds[1].left, backgrounds[1].top, backgrounds[1].right - backgrounds[1].left, backgrounds[1].bottom - backgrounds[1].top, 0xff5c96c2, 0,
        3);
...
void PauseWindow::handleClick(int id)
{
    ...
    switch (id)
    {
        ...
        case RESTART:
            bPromptToAbort = true;
            LogisticsOKDialog::instance()->setText(IDS_DIALOG_ABORT_PROMPT, IDS_DIALOG_NO, IDS_DIALOG_YES);
