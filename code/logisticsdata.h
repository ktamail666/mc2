...
public:
    ...
    void addMechToInventory(LogisticsVariant* pVar, LogisticsPilot* pPilot, int ForceGrup, bool bSubtractPts = 0);
    ...
    void setResourcePoints(long rp)
    {
        resourcePoints = rp;
        rpJustAdded    = 0;
    }
    ...
    void addResourcePoints(int amount)
    {
        resourcePoints += amount;
        rpJustAdded = true;
    }
    void decrementResourcePoints(int amount)
    {
        resourcePoints -= amount;
    }
...
private:
    ...
    //typedef std::list<LogisticsComponent> COMPONENT_LIST;
