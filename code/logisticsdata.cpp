...
void LogisticsData::init()
{
    ...
    {
        Assert(0, 0, "couldn't find the campaign file\n");
    }
    missionInfo->init(file);

    // TODO temporary, just so we can test
    int count = 32;
...
void LogisticsData::initPilots()
{
    ...
    auto pilotPath(std::string(objectPath) + "pilots.csv");

    File pilotFile;
    pilotFile.open(pilotPath.c_str());
...
int LogisticsData::purchaseMech(LogisticsVariant* pVariant)
{
    ...
    if (missionInfo->getCBills() - RP >= 0)
    {
        ...
        auto pMech = new LogisticsMech(pVariant, count);
...
int LogisticsData::getAvailableMissions(const char** missionNames, long& count)
{
    ...
    missionInfo->getAvailableMissions(nullptr, numberOfEm);
...
int LogisticsData::getCurrentMissions(const char** missionNames, long& count)
{
    ...
    // first figure out how many there are
    missionInfo->getCurrentMissions(nullptr, numberOfEm);
...
void LogisticsData::addMechToInventory(LogisticsVariant* pVar, LogisticsPilot* pPilot, int ForceGroup, bool bSubtractPts)
{
    if (pVar)
    {
...
void LogisticsData::setMissionCompleted()
{
    ...
    if (pTeam)
    {
        for (int i = pTeam->getRosterSize() - 1; i > -1; i--)
        {
            ...
            if (pMover->isOnGUI() && (pMover->getObjectType()->getObjectTypeClass() == BATTLEMECH_TYPE)
                && (pMover->getCommanderId() == Commander::home->getId()) && (pMover->getMoveType() != MOVETYPE_AIR))
            {
...
void LogisticsData::appendAvailability(const char* pFileName, bool* availableArray)
{
    ...
    for (int i = 0; i < 255; i++)
    {
        ...
        for (VARIANT_LIST::EIterator vIter = variants.Begin(); !vIter.IsDone(); vIter++)
        {
            EString mechName = (*vIter)->getFileName();
            char realName[255];
            _splitpath(mechName, NULL, nullptr, realName, NULL);
            if (S_stricmp(realName, chassisFileName) == 0)
            {
...
int LogisticsData::getChassisVariants(const LogisticsChassis* pChassis, LogisticsVariant** pVar, int& maxCount)
{
    int retVal = 0;
...
void decryptFile(char* inputFile, char* outputFile)
{
    ...
    // Since our LZ compression is pretty much non-standard, that should be enough.
    MemoryPtr rawData  = nullptr;
    MemoryPtr zlibData = nullptr;
    MemoryPtr LZData   = nullptr;
...
long LogisticsData::getCurrentMissionNum()
{
