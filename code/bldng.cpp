...
DWORD WallType[18] = { 26, 125, 152, 154, 156, 158, 160, 161, 162, 202, 204, 206, 231, 232, 233, 286, 287, 288 };
...
long Building::update(void)
{
    ...
    if (turn != updatedTurn)
    {
        ...
        if (appearance)
        {
            ...
            if (parent && (!ObjectManager->getByWatchID(parent)->isDisabled())
                && ObjectManager->getByWatchID(parent)->getTargeted())  //must do before we set selection
            {
                setTargeted(true);
            }
            ...
            appearance->setObjectParameters(
                position, ((ObjectAppearance*)appearance)->rotation, drawFlags, getTeamId(), Team::getRelation(getTeamId(), Team::home->getId()));
    ...
    if ((((BuildingTypePtr)getObjectType())->lookoutTowerRange > 0.0f) && getTeam()
        && (!parent || (parent && !ObjectManager->getByWatchID(parent)->isDisabled() && !ObjectManager->getByWatchID(parent)->isDestroyed())))
    {
    ...
    if (parent && !ObjectManager->getByWatchID(parent)->isDisabled() && !ObjectManager->getByWatchID(parent)->isDestroyed()
    ...
    if (parent
        && (ObjectManager->getByWatchID(parent)->isDisabled() || ObjectManager->getByWatchID(parent)->isDestroyed()
